                                                ...
                                                @if ($employee->attendanceToday)
                                                    <td>
                                                        <h6 class="text-center"><span
                                                                class="badge badge-pill badge-success">Terekam</span></h6>
                                                    </td>
                                                    <td>
                                                        Terekam sejak
                                                        {{ $employee->attendanceToday->created_at->format('H:i:s') }} dari
                                                        {{ $employee->attendanceToday->entry_location }} dengan alamat IP
                                                        {{ $employee->attendanceToday->entry_ip }}
                                                    </td>
                                                    <?php if($employee->attendanceToday->time<=9 && $employee->attendanceToday->time>=7) { ?>
                                                    <td>
                                                        <h6 class="text-center"><span
                                                                class="badge badge-pill badge-success">Hadir Tepat
                                                                Waktu</span></h6>
                                                    </td>
                                                    <?php } elseif ($employee->attendanceToday->time>9 && $employee->attendanceToday->time<=17) {
                                            ?><td>
                                                        <h6 class="text-center"><span
                                                                class="badge badge-pill badge-warning">Hadir
                                                                Terlambat</span></h6>
                                                    </td><?php
                                        } else {
                                           ?><td>
                                                        <h6 class="text-center"><span
                                                                class="badge badge-pill badge-danger">Absensi Tidak
                                                                Valid</span></h6>
                                                    </td><?php 
                                        } ?>
                                                    <td>
                                                ...
                                                @else
                                                    <td>
                                                        <h6 class="text-center"><span
                                                                class="badge badge-pill badge-danger">Belum Ada
                                                                Riwayat</span></h6>
                                                    </td>
                                                    <td>
                                                        <h6 class="text-center"><span
                                                ...
                                                <td>
                                                    ...
                                                    $employee_id = $employee->id; // Ambil ID karyawan
                                                    $loc2 = mysqli_query($conn, "SELECT * FROM attendances WHERE employee_id = $employee_id");
                                                    while ($loc = mysqli_fetch_array($loc2)) {
                                                        if (!empty($loc['entry_location'])) {
                                                            echo 'Absen Masuk : '.$loc['entry_location'];
                                                            echo '<br>';
                                                            echo 'Absen Keluar : '.$loc['exit_location'];
                                                            echo "<hr>"; 
                                                        } else {
                                                            echo ' - ';
                                                        }
                                                ...
                                                <td>
                                                    @if ($employee->attendanceToday)
                                                        <button class="btn btn-flat btn-danger" data-toggle="modal"
                                                            data-target="#deleteModalCenter{{ $employee->attendanceToday->id }}">Hapus
                                                            Riwayat</button>
                                                    @else
                                                        Aksi Tidak Tersedia
                                                    @endif
                                ...
                                @for ($i = 1; $i < $employees->count() + 1; $i++)
                                    ...
                                    @if ($employees->get($i - 1)->attendanceToday)
                                        <div class="modal fade"
                                            ...
                                            aria-labelledby="deleteModalCenterTitle1{{ $employees->get($i - 1)->attendanceToday->id }}"
                                            aria-hidden="true">
                                            <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
                                                <div class="modal-content">
                                                    <div class="card card-danger">
                                                        <div class="card-header">
                                                            <h5 style="text-align: center !important">Yakin ingin dihapus?
                                                            </h5>
                                                        </div>
                                                        <div class="card-body text-center d-flex"
                                                            ...
                                                            <form
                                                                ...
                                                                @method('DELETE')
                                                                <button type="submit"
                                                                    class="btn flat btn-danger ml-1">Ya</button>
                                                            </form>
                                                        </div>
                                                        <div class="card-footer text-center">
